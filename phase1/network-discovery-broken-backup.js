...
<e:Header><w:MessageID>${uuid}</w:MessageID><w:To>urn:schemas-xmlsoap-org:ws:2005:04:discovery</w:To><w:Action>http://schemas.xmlsoap.org/ws/2005/04/discovery/Probe</w:Action></e:Header>
<e:Body><d:Probe><d:Types>dn:NetworkVideoTransmitter</d:Types></d:Probe></e:Body></e:Envelope>`.replace("xmlns:dn=", 'xmlns:dn="http://www.onvif.org/ver10/network/wsdl" xmlns:d=');
    ...
    sock.on("message", msgBuf => {
      const xml = msgBuf.toString();
      const xaddrs = allTags(xml, "XAddrs").flatMap(x => x.split(/\s+/)).filter(Boolean);
      const scopes = allTags(xml, "Scopes");
      const endpoint = xmlTag(xml, "Address");
      const first = xaddrs[0] || "";
      const key = first || endpoint || ip;
      responses.set(key, {
        ip,
        xaddrs: [...new Set(xaddrs)],
        scopes: scopes.flatMap(x => x.split(/\s+/)),
        source: "onvif"
      });
    });
    ...
    sock.bind(0, ifaceAddress, () => {
      try { sock.setMulticastInterface(ifaceAddress); } catch {}
      const buf = Buffer.from(msg);
      sock.send(buf, 0, buf.length, ONVIF_UDP_PORT, "239.255.255.250");
      // Also unicast to the target when supplied; useful on some networks.
      if (ip) sock.send(buf, 0, buf.length, ONVIF_UDP_PORT, ip);
    });
